...
tab_eps0 = "./par_eps_09_03_pixdim3.txt"
...
data_address = 'C:/Users/Felipe Casarin/Desktop/Incidence-Position-On-Phoswich-Detector/collection_files_csv_unfiltered/-4_2.csv'
...
def fitpatxy(): # pattern fit of x,y
    ...
    IM.eps=np.loadtxt("par_eps_09_03_pixdim3.txt")
...
def pos0(): 
    ...
    print("It is done.")
    response.to_excel('-4_2_pixdim3_05_03.xlsx', index=False)

def pos(x,y): 

    data_address = 'C:/Users/Felipe Casarin/Desktop/Incidence-Position-On-Phoswich-Detector/collection_files_csv_unfiltered/' + str(x) +'_' + str(y) +'.csv'
    data=pd.read_csv(data_address)
    ...
    for entryNum in range(0, len(data)):    
        ...
        if L1>0 and L2>0 and L3>0 and L4>0 and C1>0 and C2>0 and C3>0 and C4>0:
            ...
            response = response._append(new_row, ignore_index=True)
    print("It is done.")
    response.to_excel('outfiles_pos_calc_0903_pixdim3/' + str(x) + '_' + str(y) + '_pos' + '.xlsx', index=False)
...
def pos_all():
    directory = os.fsencode('C:/Users/Felipe Casarin/Desktop/Incidence-Position-On-Phoswich-Detector/row_line_inversion_pha/')
    for file in os.listdir(directory):
        filename = os.fsdecode(file)
        if not filename.endswith('.xlsx'):
            print(f"Skipping {filename}: Not a .xlsx file.")
            continue
        data_address = 'C:/Users/Felipe Casarin/Desktop/Incidence-Position-On-Phoswich-Detector/row_line_inversion_pha_6/' + filename
        data=pd.read_excel(data_address)
        ...
        print("It is done.")
        response.to_excel('C:/Users/Felipe Casarin/Desktop/Incidence-Position-On-Phoswich-Detector/row_line_inversion_pha_6/outfiles_0103/output_'+ filename, index=False)
